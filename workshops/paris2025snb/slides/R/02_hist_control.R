...
pl_map <- pl_map_AS <- plot(map_mc, size = 1.5)$forest_model +
  xlab(NULL) +
  ylab("Responder rate")
...
## -----------------------------------------------------------------------------
#| output: false
form_AS <- bf(r | trials(n) ~ 1 + (1 | study), family = binomial("logit"))
...
## -----------------------------------------------------------------------------
AS_region_new <- data.frame(study = "new_study_asia", n = 1, region = "asia")
