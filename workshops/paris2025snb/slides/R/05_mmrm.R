...
ggplot(design, aes(x = x, y = y, label = arm, fill = arm)) +
  ...
  geom_text(
    data = design |> filter(x == 1 & y %in% c(1:10)),
    aes(x = x + 0.1, y = y - 0.3),
    size = 10,
    hjust = 0,
    vjust = 0.75
...
simulated_data |>
  ...
  tab_style(
    locations = cells_body(rows = c(4, 8, 10)),
    style = cell_borders(
      sides = "bottom",
      color = "black",
      weight = px(1.5),
...
#| echo: false
map2_dfr(
  list(mmrm_fit, fit_mmrm1, fit_mmrm2),
  ...
  mutate(
    Week = c(2L, 4L, 8L, 12L)[as.integer(str_extract(AVISIT, "[0-9]+"))],
    Model = factor(Model, c("Frequentist", "Bayesian", "Bayesian (monotonic)")),
