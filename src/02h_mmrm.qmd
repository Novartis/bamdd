...
plasma glucose (FPG) values from one treatment group of a diabetes trial.
...
[R/PHARMA presentation on implementing MMRM in R by Daniel Sabanés Bové](https://rinpharma.com/publication/rinpharma_143/).
```{r lmermodel_example}
...
"distributional regression" (i.e. we can specify a regression model
for parameters of the distribution other than the location, in this
case `sigma`).
...
reflect the correlation between the predictions for the different parameters.
