...
library(survminer)
library(gt)
here::i_am("src/02i_time_to_event.qmd")
...
### Model specification {#sec-tte-model-spec}
...
gt(tb1) |>
  cols_label(activeChemoA  = md("$\\lambda_{\\mbox{activeChemoA}}$"),
             controlChemoA = md("$\\lambda_{\\mbox{controlChemoA}}$"),
             activeChemoB  = md("$\\lambda_{\\mbox{activeChemoB}}$"),
             controlChemoB = md("$\\lambda_{\\mbox{controlChemoB}}$")
             ) |>
...
gt(tb2) |>
  cols_label(TreatmentArm = "Treament arm",
             intercept = md("$\\gamma$"),
             effectAvg = md("$\\delta_{\\mbox{EffectAvg}}$"),
             effect = md("$\\delta_{\\mbox{effect}}$"),
             control = md("$\\delta_{\\mbox{control}}$")) |>
  fmt_markdown(everything())
```
...
substantial deviation from unity, which is the case of an exponential
...
of the model as defined in @sec-tte-model-spec.
...
Consider using the `exponential` distribution instead of the used
`weibull` distribution. Evaluate first how well the simpler
`exponential` model describes the historical data using a posterior
predictive check. Then reconsider the borrowing model presented.
...
as detailled in @sec-tte-model-brms. 
